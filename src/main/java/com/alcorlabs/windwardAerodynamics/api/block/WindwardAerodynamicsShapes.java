...
public class WindwardAerodynamicsShapes {
    ...
    static {
        ...
        for (Direction facing : Direction.values()) {
            for (int chordRot = 0; chordRot < 4; chordRot++) {
                ...
                // We need to rotate the base shape (Normal=UP, Chord=NORTH) 
                // into the target state (Normal=facing, Chord=chord).
                
                // The base normal is UP. Base chord is NORTH.
                // Cross product of UP and NORTH gives EAST (+X).
                // Target normal is facing. Target chord is chord.
                // Cross product of facing and chord gives the new "Right" vector.
                
                // Let's build a 3x3 rotation matrix that maps:
                // Base Right (+X) -> Target Right
                // Base Normal (+Y) -> Target Normal
                // Base Chord (-Z) -> Target Chord
                // Note: -Z is North. Target Chord is what the original North (-Z) face maps to!
                // So Base Z-axis (+Z, South) -> Target Chord's OPPOSITE!
                
                // Let's compute Target Right (X-axis in base) = Target Normal x Target Chord (Wait, UP x NORTH = +Y x -Z = -X?
                // Let's check: +Y x -Z. i j k. 0 1 0 x 0 0 -1. i(1*-1 - 0) = -1! So UP x NORTH = WEST (-X).
                // So base "Left" (+X) -> Target Left?
                // Actually, let's just rotate each of the 8 corners of the AABB and find min/max.
    ...
    private static VoxelShape rotateShape(VoxelShape shape, Direction targetNormal, Direction targetChord) {
        ...
        // Wait, standard Minecraft: X x Y = Z?
        // Right hand: X=East, Y=Up, Z=South. 
        // East(+X) x Up(+Y) = South(+Z). Yes.
        // So Target X = Target Y.getCrossProduct(Target Z)?
        // Wait, Minecraft doesn't have getCrossProduct on Direction.
        // Let's write a quick cross product for Direction!
